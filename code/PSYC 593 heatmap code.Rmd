...
# read in data
et_data <- read.asc(file.path(eyetracking_fv_data_path, "EXP1_freeview_15_2023_02_11_15_04.asc"), parse_all = TRUE)
...
trial_bounds <- msgs %>% 
                  filter((text == "stimuli_onset" | text == "freeview_timeup") & block > 24) 
...
make_trials <- function(eyetrack_data, bounds) {
  ...
  bounds <- bounds %>%
              ...
              select(id, stimuli_onset, freeview_timeup)
...
trials_df <- subset(trials_df, trial_id == 1)%>%
                filter(row_number() %% 50 == 0)
